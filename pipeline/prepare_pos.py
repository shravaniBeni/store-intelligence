...
def normalize_pos(input_path: Path, output_path: Path) -> int:
    ...
    with input_path.open("r", encoding="utf-8-sig", newline="") as handle:
        ...
        if not reader.fieldnames:
            raise ValueError("POS file is missing a header row")
        if "invoice_number" in reader.fieldnames:
            transaction_key = "invoice_number"
        elif "order_id" in reader.fieldnames:
            transaction_key = "order_id"
        else:
            raise ValueError("POS file must include invoice_number or order_id")
        for row in reader:
            transaction_id = row[transaction_key]
            ...
            orders[transaction_id]["basket_value_inr"] += float(row["total_amount"] or 0)
